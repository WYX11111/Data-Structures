...
const LISTINCREMENT=sizeof(int);
...
int GetElem(SqList L,int i,int e) //取得元素
{
	if(i<1||i>L.length)
        cout << "无第" << i << "个数" <<endl;
    int *p = L.elem;
...
int LocateElem(SqList L,int e) //确定位置
{
    int i = 1;
    int *p = L.elem;
...
int PriorElem(SqList L,int cur_e,int pre_e)  //寻找前一个元素
{
    int *p = L.elem;
    int i = 1;
...
int NextElem(SqList L,int cur_e,int next_e) //寻找下一个元素
{
    int *p = L.elem;
    int i = 1;
	while(i <= L.length && *p++ != cur_e)
